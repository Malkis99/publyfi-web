...
const RewardItem = ({ image, label, delay }: { image: string, label: string, delay: number }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay }}
    className="flex flex-col items-center text-center p-3 bg-black/20 rounded-lg border border-white/10"
  >
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {rewardItems.map((item, index) => (
            <RewardItem key={item.label} {...item} delay={(index + 1) * 0.1} />
          ))}
        </div>
