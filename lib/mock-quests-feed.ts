...
export interface LiveStreamerFeedItem {
  type: FeedItemType.LiveStreamer;
  id: string;
  streamer: {
    name: string;
    avatar: string;
...
export interface CommunityHighlightFeedItem {
    type: FeedItemType.CommunityHighlight;
    id: string;
    text: string;
}

export interface GuildBriefingFeedItem {
    type: FeedItemType.GuildBriefing;
    id: string;
    text: string;
}

export type FeedItem = LoreFeedItem | StreamerActivityFeedItem | TopQuestFeedItem | PlayerActivityFeedItem | FeaturedQuestFeedItem | LiveStreamerFeedItem | CommunityHighlightFeedItem | GuildBriefingFeedItem;

// Helper function to create a feed item and ensure the quest exists
const createFeedItem = (item: Omit<StreamerActivityFeedItem, 'quest' | 'id'> & { questId: string, id: string } | Omit<TopQuestFeedItem, 'quest' | 'id'> & { questId: string, id: string } | Omit<PlayerActivityFeedItem, 'quest' | 'id'> & { questId: string, id: string } | Omit<FeaturedQuestFeedItem, 'quest' | 'id'> & { questId: string, id: string } | Omit<LiveStreamerFeedItem, 'quest' | 'id'> & { questId: string, id: string } ): FeedItem | null => {
...
const feedItemDefinitions = [
  ...
  createFeedItem({
   ...
   createFeedItem({
    ...
    type: FeedItemType.GuildBriefing,
    id: 'guild-briefing-1',
    text: "Rumor: A new Mythic Contract unlocks in 12 hours…",
  },
  createFeedItem({
    type: FeedItemType.LiveStreamer,
    id: 'live-streamer-1',
    streamer: { name: 'MiraX', avatar: 'M' },
    questId: 'rare002',
  }),
  {
    type: FeedItemType.CommunityHighlight,
    id: 'community-highlight-1',
    text: "@Nova discovered a hidden chain quest: Signal Collapse.",
  },
];
