...
**Key Features:**
*   The avatar is a fully animated 3D model rendered via `Unity`.
*   It is customizable with skins, armor, weapons, emotes, and visual effects.
*   It evolves visually with level, reputation, achievements, and rarity.
*   It is linked to the user's wallet and on-chain activity, forming a digital legacy.
...
With `GameSync`:
*   `XP`, quests, tokens, and inventory follow users across modules (streams, `PVP`, quests, social).
*   The avatar “remembers” user actions, unlocking gear and interactions.
*   It enables seamless onboarding to external games via `API` integrations.
...
**Core Features:**
*   Users can earn gear via quests, purchase, or reputation.
*   Each piece is an `ERC-721` `NFT` on `Immutable zkEVM`.
*   Special sets unlock emotes, animations, and `VFX` trails.
*   Users can modify interactions and appearance based on mood, `XP`, and faction.
*   Items are tradable on the internal marketplace or external `L2s`.
